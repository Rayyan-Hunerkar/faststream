...
from server.models.auth import AccessToken, RefreshToken
from server.models.users import User, UserAuth
...
async def login(user_auth: UserAuth, auth: AuthJWT = Depends()):
    ...
    if user is None or verify_password(user_auth.password, user.password) is False:
